...
| **Multiple tickers sharing one filing** | When a 485BPOS genuinely covers multiple series — e.g. the Vanguard Index Funds document shared by VOO and VTSAX under CIK 36405 — the series-scoped Atom feed correctly surfaces the same accession (`0000036405-26-000181`) under both series. An in-run accession cache prevents double-downloading: the second ticker reuses the already-saved path and sha256, and a separate manifest entry is written for each ticker. |
| **Multiple series, one combined statutory prospectus** | The Vanguard Index Funds trust (CIK 36405) files a single 12-series 485BPOS every year. VOO (`S000002839`) and VTSAX (`S000002848`) — along with ten other equity index funds — all receive the same accession (`0000036405-26-000181`) from their respective series feeds, because EDGAR registers the filing under all covered series simultaneously. An in-run accession cache prevents double-downloading; a manifest entry is written per ticker. Content validation confirms the series/class IDs for each ticker appear in the document and records the evidence. |
