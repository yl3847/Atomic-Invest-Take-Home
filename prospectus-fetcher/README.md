...
**Why series-scoping gives precise accession selection:** Querying the EDGAR browse-Atom feed by series ID returns only the filings associated with that series — not all filings for the registrant CIK. This is visible in real output: `inspect VUSXX VMFXX` returns accessions `…325143` and `…325144` respectively, even though both are filed by the same trust on the same date. The feed hands each series only the filing(s) that cover it. Where a single combined 485BPOS covers multiple series (e.g. the Vanguard Index Funds document shared by VOO and VTSAX under CIK 36405), that accession surfaces under both series — which is exactly right. Series-scoping selects the correct *accession*; content validation independently confirms the series/class IDs appear in the downloaded document.
...
| **Series-scoped Atom feed for precise accession selection + content validation as an independent confirmation** | The EDGAR browse-Atom feed, queried by series ID, returns only the filings associated with that series. This gives exact per-fund accession selection: VUSXX (`S000002233`) and VMFXX (`S000004462`) both resolve to CIK 891190 but receive adjacent, distinct accessions (`…325143` vs `…325144`) on the same filing date — the feed is filtering per series, not dumping all filings for the trust. Where one 485BPOS genuinely covers multiple series (e.g. VOO and VTSAX share `0000036405-26-000181`), the same accession correctly surfaces under both. Content validation (scanning the downloaded HTML for the series/class IDs) is a second, independent layer that does a different job: it confirms the filing's *content* covers this fund and records the evidence in the manifest for auditability. The two layers are complementary — selection picks the right accession, validation confirms the right content — and neither substitutes for the other. Validation is recorded, never a gate; the file is kept even on a miss because a scan failure does not imply a wrong document. |
...
| **Per-series filings in multi-fund trusts** | Where a trust files separate 485BPOS documents per fund — as Vanguard Admiral Funds does — the series-scoped feed returns only the accession for that series. VUSXX (`S000002233`) and VMFXX (`S000004462`) retrieve adjacent accessions `…325143` and `…325144` on the same date; neither sees the other's filing. Content validation then independently confirms the series/class IDs appear in the downloaded document. |
