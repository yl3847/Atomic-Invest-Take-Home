...
| **ETF vs mutual fund routing** | `company_tickers_mf.json` covers Investment Company Act registrants and includes series/class IDs. `company_tickers.json` covers exchange-listed securities more broadly but lacks series/class IDs. SPY (CIK 884394) resolves via the company map (no series/class IDs) and is selected via the submissions API; VOO (CIK 36405, series `S000002839`) and QQQ (CIK 1067839, series `S000101292`) both resolve via the MF map and are selected via the series-scoped Atom feed. The three-tier chain handles all three cases transparently. |
