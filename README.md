...
**Registrant (trust / CIK)** — the legal entity that files with the SEC. A single CIK can cover dozens of funds. Vanguard Admiral Funds files under CIK 891190; Vanguard Index Funds files separately under CIK 36405 and covers VOO, VTSAX, and others.

**Series (S-id)** — one fund within a registrant. VUSXX is series `S000002233` (Vanguard Treasury Money Market Fund) under CIK 891190. VMFXX is a separate registrant entirely: CIK 106830, series `S000004462` (Vanguard Federal Money Market Fund). The series is the economic unit — it has its own NAV, holdings, and prospectus. A filing appears under every series it covers, which is how EDGAR associates a multi-series document with each fund.
...
**Why series-scoping gives precise accession selection:** Querying the EDGAR browse-Atom feed by series ID returns every filing EDGAR has registered under that series — not all filings for the registrant CIK. Where one 485BPOS genuinely covers multiple series (e.g. the Vanguard Index Funds document shared by VOO and VTSAX under CIK 36405), the same accession (`0000036405-26-000181`) surfaces under both series feeds — which is exactly right. `inspect VOO VTSAX` confirms both series independently select the same accession; the document is a 12-series combined statutory prospectus that EDGAR has registered under all 12 series. Where a trust files separate per-series documents, each series' feed returns only its own accession. Series-scoping selects the correct *accession*; content validation independently confirms the series/class IDs appear in the downloaded document, which is the only definitive per-fund check.

VUSXX and VMFXX illustrate a different structural point: they are registered under *different* trusts. VUSXX is series `S000002233` under CIK 891190 (Vanguard Admiral Funds), while VMFXX is series `S000004462` under CIK 106830 (Vanguard Money Market Reserves) — a separate legal entity filing separately with the SEC. Their adjacent accessions (`…325143` and `…325144`) simply reflect two registrants filing on the same date, not series-level splitting within one trust.
...
| **Series-scoped Atom feed for accession selection + content validation as the per-fund correctness guarantee** | The EDGAR browse-Atom feed, queried by series ID, returns every filing EDGAR has registered under that series. Where one 485BPOS covers multiple series — e.g. `0000036405-26-000181` is a 12-series combined statutory prospectus registered under VOO, VTSAX, and ten other Vanguard Index Funds series — the same accession surfaces correctly under all of them. Selection picks the right accession; content validation is the independent layer that confirms the downloaded document actually covers *this specific fund*. It scans the HTML for the fund's EDGAR-assigned series_id and class_id — identifiers that are unique per share class and cannot appear in a wrong-fund document. A series/class ID match (`passed=True`) is a definitive per-fund signal; a ticker-only match is flagged as a weak signal (`passed=False`) with a note recommending manual verification. Both layers are complementary and neither substitutes for the other. Validation is recorded in the manifest for auditability; the file is kept on a miss because a scan failure does not imply a wrong document. |
...
| **ETF vs mutual fund routing** | `company_tickers_mf.json` covers Investment Company Act registrants and includes series/class IDs. `company_tickers.json` covers exchange-listed securities more broadly but lacks series/class IDs. SPY (CIK 884394) is a unit investment trust with no EDGAR series ID; it resolves via the company map and is selected via the submissions API. VOO (CIK 36405, series `S000002839`) and QQQ (CIK 1067839, series `S000101292`) both resolve via the MF map and are selected via the series-scoped Atom feed. Note: QQQ converted from a unit investment trust to an open-end management investment company effective December 2025 and now carries an EDGAR series ID, which is why it routes through the MF map rather than alongside SPY. The three-tier chain handles all three cases transparently. |
...
| **Multiple tickers sharing one combined filing** | The Vanguard Index Funds trust (CIK 36405) files a single combined 485BPOS covering multiple equity-index series each year. VOO (`S000002839`) and VTSAX (`S000002848`) each query their own series feed and both receive the same accession (`0000036405-26-000181`), because EDGAR registers the filing under all covered series simultaneously. An in-run accession cache downloads the document once; the second ticker reuses the already-saved path and sha256, and a separate manifest entry is written per ticker. Content validation then independently confirms each fund's series/class ID appears in the document. |
...
... (45 tests total)
45 passed, 3 deselected in 0.14s
```

Tests cover:

- **URL builders** — exact URL shapes for all 5 EDGAR endpoints
- **Resolver** — MF-map hit (VUSXX), company-map hit (SPY — unit investment trust with no series ID), full-text fallback, ticker normalization, `TickerNotFound`
...
- **Validation** — strong signals (series/class ID), ticker-only weak signal (passed=False), no-signal failure, missing file, cross-fund isolation (fund A's IDs in doc → fund B fails; combined doc with both IDs → both pass)
